...
class phmm:
    ...
    def transition(self):
        ...
        for trans_p in transition_counts:
            temp = {}
            tot_m = sum([value for key, value in trans_p.items() if key[0] == "M"])
            tot_d = sum([value for key, value in trans_p.items() if key[0] == "D"])
            tot_i = sum([value for key, value in trans_p.items() if key[0] == "I"])
            for key, value in trans_p.items():
                if key[0] == "M" and tot_m != 0:
                    temp[key] = value/tot_m
                elif key[0] == "D" and tot_d != 0:
                    temp[key] = value/tot_d
                elif key[0] == "I" and tot_i != 0:
                    temp[key] = value/tot_i
            prob_t.append(temp)
        return state_prob, prob_t
    
    def check_transition(self):
        # check if the sum of the probabilities of each transition from a state is 1
        for i in self.transition_prob:
            tot_m = int(sum([value for key, value in i.items() if key[0] == "M"]))
            tot_d = int(sum([value for key, value in i.items() if key[0] == "D"]))
            tot_i = int(sum([value for key, value in i.items() if key[0] == "I"]))
        if (tot_m != 1 and tot_m != 0) and  (tot_d != 1 and tot_d != 0) and (tot_i != 1 and tot_i != 0):
            return False
        return True
